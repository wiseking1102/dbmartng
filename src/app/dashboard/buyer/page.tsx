...
export default function BuyerDashboardPage() {
   ...
   * Buyer-only access.
   *
   * Buyers may enter this dashboard.
   * Vendors go to the vendor dashboard.
   * Admins/sub-admins go to the admin dashboard.
   */
  useEffect(() => {
    if (loading) return;
  ...
  /*
   * Only fetch buyer-specific data for an authenticated buyer.
   */
  useEffect(() => {
    ...
    const fetchData = async () => {
      try {
        ...
        if (searchesRes.ok) {
          ...
          if (
            !cancelled &&
            Array.isArray(searchesData?.data)
          ) {
            setSavedSearchCount(searchesData.data.length);
          }
        }
        ...
        if (referralsRes.ok) {
          ...
          if (referralsData?.stats) {
            const referralData = Array.isArray(referralsData.data)
              ? referralsData.data
              : [];

            const rewarded = referralData.filter(
              (referral: ReferralReward) =>
                referral.status === "rewarded"
            ).length;

            setReferralStats({
              total: Number(referralsData.stats.total ?? 0),
              converted: Number(
                referralsData.stats.converted ?? 0
              ),
              pending: Number(
        ...
        /*
         * Keep the dashboard usable even if optional
         * buyer data fails to load.
         */
      }
    };
  ...
  /*
   * Keep this hook above all conditional returns.
  ...
  /*
   * While authentication/role resolution is happening,
   * don't render buyer-specific content.
   */
  if (loading || !user || role !== "buyer") {
    return (
      <>
        ...
        <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8 py-8">
          <StaggerEntrance>
            ...
            <div className="mb-8">
              <div className="flex items-center justify-between mb-4">
                ...
                {referralStats.total > 0 && (
                  <span className="text-sm text-gray-500">
                    {referralStats.total} referral
                    {referralStats.total !== 1 ? "s" : ""} sent
                  </span>
                )}
              </div>
              ...
              {/* Credits Balance + Reward History */}
                    ...
                    <div className="flex items-center justify-between">
                      ...
                      <div className="text-right">
                        <p className="text-sm font-medium text-brand-navy">
                          {referralStats.rewarded} reward
                          {referralStats.rewarded !== 1
                            ? "s"
                            : ""}{" "}
                          earned
                        </p>
            ...
            <div className="glass rounded-2xl p-8 text-center bg-gradient-to-br from-brand-navy/5 to-brand-gold/5">
              ...
              <p className="text-gray-600 mb-6 max-w-lg mx-auto">
                Browse hundreds of verified vendors across Nigeria. Find
